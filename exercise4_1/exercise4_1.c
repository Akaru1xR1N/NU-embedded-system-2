...
void main(){ // ฟังก์ชันหลัก
	...
	while (1){ // infinite loop
		if (sw0 == 1){ // ถ้า sw0 == 0
			printf("Thanadej"); // ส่งข้อความ Thanadej ผ่านทาง serial port
			delay(50000); // ตั้งค่า delay
		}
		else if (sw1 == 1){ // ถ้า sw1 == 0
			printf("Phrompornchai"); // ส่งข้อความ Phrompornchai ผ่านทาง serial port
			delay(50000); // ตั้งค่า delay
		}
		else if (sw2 == 1){ // ถ้า sw2 == 0
			printf("64362609"); // ส่งข้อความ 64362609 ผ่านทาง serial port
			delay(50000); // ตั้งค่า delay
		}
	}
}
